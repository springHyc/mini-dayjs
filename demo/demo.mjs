...
console.log('    a. dayjs("2024-06-15T14:30:00").format(...)');
console.log('        =>', d.format('YYYY年MM月DD日 HH:mm'));

title('2. 不可变：add 返回新实例，原实例不变');
const a = dayjs('2024-01-10');
const b = a.add(1, 'M');
console.log('    a. a =', a.format('YYYY-MM-DD'));
console.log('    b. b = a.add(1, "M") =>', b.format('YYYY-MM-DD'));
const t0 = dayjs('2024-01-10 10:00:00');
console.log(
    '    c. dayjs("2024-01-10 10:00:00").add(2, "d")（按天：走毫秒换算）',
    '=>',
    t0.add(2, 'd').format('YYYY-MM-DD HH:mm:ss')
);
console.log(
    '    d. dayjs("2024-01-10 10:00:00").add(2, "h")（按小时：走毫秒换算）',
    '=>',
    t0.add(2, 'h').format('YYYY-MM-DD HH:mm:ss')
);
console.log(
    '    e. dayjs("2024-01-10 10:00:00").add(1, "w")（按小时：走毫秒换算）',
    '=>',
    t0.add(1, 'w').format('YYYY-MM-DD HH:mm:ss')
);

title('3. 月/年：日历语义（与 dayjs 一致）—— 月末 +1 月落在目标月最后一天（如 1/31 +1M → 2/29）');
console.log('    a. dayjs("2024-01-31").add(1, "M").format("YYYY-MM-DD")');
console.log('        =>', dayjs('2024-01-31').add(1, 'M').format('YYYY-MM-DD'));

title('4. startOf / endOf —— 聚合统计常用');
console.log('    a. startOf("M"):', dayjs().startOf('M').format('YYYY-MM-DD HH:mm:ss'));
console.log('    b. endOf("y"):', dayjs('2024-06-15').endOf('y').format('YYYY-MM-DD HH:mm:ss'));

title('5. 插件：extend 之后再调用 fromNow / from / toNow');
extend(relativeTime);
...
console.log('    d. almost2minLater.fromNow() =>', almost2minLater.fromNow());

title('6. 比较');
const cmpA = dayjs('2024-01-01');
const cmpB = dayjs('2024-01-01T23:59:59');
const sameDay = cmpA.isSame(cmpB, 'd');
console.log('    a. cmpA = dayjs("2024-01-01")        =>', cmpA.format('YYYY-MM-DD HH:mm:ss'));
console.log('    b. cmpB = dayjs("2024-01-01T23:59:59") =>', cmpB.format('YYYY-MM-DD HH:mm:ss'));
console.log('    c. cmpA.isSame(cmpB, "d")（按「天」比较是否同一天）=>', sameDay);
...
console.log(
    '    a. dayjs("2024-06-15 12:00:00").startOf("d").format("HH:mm:ss")',
    '=>',
    dRange.startOf('d').format('HH:mm:ss')
);
console.log(
    '    b. dayjs("2024-06-15 12:00:00").endOf("d").format("HH:mm:ss")',
    '=>',
...
console.log('    dayjs(Date.now() - 55000).fromNow()', '=>', almost1minAgo.fromNow());
